...
$MTU=Find-MTU 8.8.8.8

Set-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters -Name MTU -Value $MTU -ErrorAction SilentlyContinue
New-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters -PropertyType DWord -Name MTU -Value $MTU -ErrorAction SilentlyContinue

$cles=Get-ChildItem -Path HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters\Interfaces

$cles | ForEach-Object {
	$DhcpServer=Get-ItemProperty -Path $_.PSPath -Name DhcpServer -ErrorAction SilentlyContinue
	If($DhcpServer) {
		If($DhcpServer.DhcpServer -ne "255.255.255.255") {
			Write-Host $DhcpServer.DhcpServer
			Set-ItemProperty -Path $_.PSPath -Name MTU -Value $MTU -ErrorAction SilentlyContinue
			New-ItemProperty -Path $_.PSPath -PropertyType DWord -Name MTU -Value $MTU -ErrorAction SilentlyContinue

			Set-ItemProperty -Path $_.PSPath -Name EnablePMTUDiscovery -Value 1 -ErrorAction SilentlyContinue
			New-ItemProperty -Path $_.PSPath -PropertyType DWord -Name EnablePMTUDiscovery -Value 1 -ErrorAction SilentlyContinue
		}
	}
}
